...
- WIFI 轮询复选框 `poll-wifi` 默认勾选（此前默认不勾选）。
- 纯前端改动，刷新页面即生效；历史回填（`fillFormFromHistory`）仍以保存的 platforms/pollWifi 为准，不受默认值覆盖。

---

## [0.3.34] — 2026-07-27

### Added（谷歌阶段失败节点自动换备选节点补跑）
- **背景**：压测中偶发单个 VPN 节点被 Google 限流（如 `f2fcabbe` 的 `[US]美国直连US1`），原逻辑将该节点记为 failed、不重试，整阶段完成率随之下降。
- **重构 `scripts/worker.js` 谷歌阶段轮询模型**：由「逐个节点各跑一次」改为「**目标成功数 + 失败自动换备选节点补跑**」：
  - 维护未尝试的备用节点池；某节点失败 → 记 failed → 自动从池中取下个节点补跑，**直到成功数达标或池耗尽**；
  - 整阶段终态判定：尝试数达 `targetCount` 且至少成功 1 个 → 视为完成（单节点失败已被补跑替代，不再拉低整阶段）；出现真实缺口（可用池不足）或零成功 → 阶段 FAILED。
  - 报告 `VPN_INFO` 增加 `usedCount` / `targetCount`，统计「选用节点」改为**真实首个成功节点**（修正旧版硬编码 `available[0]` 在受 `AUTOCLAW_GOOGLE_MAX_NODES` 限制时显示不准）。
- **`core/taskStats.js`**：报告「选用节点」行展示「成功 X / 目标 Y 个」及补跑提示。
- **测试**：`test/phasedTask.test.js` 新增 3 例（失败自动换节点补跑达标 / 全失败判 FAILED / MAX_NODES 下 usedNode 为首个成功节点）。全测 297/0 通过。

### Changed
- 谷歌阶段进度消息带「剩余备选 N 个」提示，便于观察补跑过程。
...
### Improved（谷歌定位失败诊断加固）
- **背景**：实测任务 `f2fcabbe` 谷歌阶段 16 节点中仅 `[US]美国直连US1` 失败（扫描 5 页均「0 外链」），其余 15 节点全命中。根因为该节点出口 IP 区域下 Google 返回零结果/受限结果页（非代码缺陷）。
- **加固 `core/adapters/googleAdapter.js` 的 `locateTarget`**：
  - 翻页 `page.goto(nextUrl)` 之后**重新检测验证码/同意页**，命中即抛明确的 `ERR_GOOGLE_CAPTCHA` / `ERR_GOOGLE_CONSENT`，不再被误判为「0 外链」；
  - 当全页解析不到任何外链时，错误信息**附带当前页 URL + 页面文本片段**，明确指向「该节点出口 IP 区域下 Google 返回零结果/受限结果页或拦截页」，消除原「可能结果页结构变化」的歧义。

---

## [0.3.32] — 2026-07-25

### Fixed（前端「完成度总结」只显示最后阶段）
- **根因**：`public/js/progress.js` 的 `renderSummary` 在每次收到 `task_stats` 事件时执行 `$summary.innerHTML = ...` **整段覆盖**容器。百度+谷歌分阶段任务会推送两条 `task_stats`（顺序 `['baidu','google']`），谷歌后到直接把百度的总结卡片覆盖掉，页面只剩谷歌。
- **修复**：
  - `renderSummary` 改为**每个平台（百度/谷歌）生成一张独立卡片并累计追加**，不再整段覆盖；
  - 卡片标题带平台标签（「📌 百度 阶段 / 📌 谷歌 阶段」），两张卡视觉分隔（新增 `.summary-card` 样式）；
  - 卡片展示 `命中目标率` 字段，逐 WIFI 明细补全「命中目标 ✅ / ⚠️未命中」标记；
  - 加平台级去重（`renderedSummaryPlatforms`），历史回看/断线重连重复收到同平台 `task_stats` 也不会重复渲染。
- 纯前端改动，刷新进度页即生效，无需重启后端。
...
### Fixed
- **谷歌适配器结果定位漏匹配（v0.3.24 后仍存在的问题）**：`locateTarget` 原来只取
  结果前 10 条（`anchors.slice(0, 10)`）且从不翻页，导致目标站点排在位置 11 及之后
  （即「页面最低端」看到的正确页面）时无法命中。本次修复：
  - 移除前 10 条限制，扫描**当前页全部结果**；
  - 定位前先滚到页面底部，触发 Google 懒加载追加靠后的结果块；
  - 本页未命中时自动翻「下一页」（`a#pnnext` / 带 aria-label 的 `start=` 链接），
    最多扫描 `MAX_RESULT_PAGES=5` 页；
  - 诊断信息改为「已扫描的 N 页」口径。
- 新增 googleAdapter 单测：超出前 10 条仍命中、翻到第 2 页命中；全量 275/1skip 通过。
